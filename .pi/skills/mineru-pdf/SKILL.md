...
| 来源 | 环境变量 | 说明 | 限制 |
|------|----------|------|------|
| **cloud** | `MINERU_API_SOURCE=cloud` | MinerU 云端精准 API（默认） | 需 Token，200MB/200页 |
| **modal** | `MINERU_API_SOURCE=modal` | Modal 本地部署 | 仅本地文件 |
| **agent** | `MINERU_API_SOURCE=agent` | MinerU Agent API | 免登录，10MB/20页 |

切换方式：
```bash
# .env 文件
MINERU_API_SOURCE=cloud  # 或 modal 或 agent

# 命令行参数
python parse_pdf.py paper.pdf --api cloud
```
